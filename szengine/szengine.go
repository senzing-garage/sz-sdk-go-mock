...
func (client *Szengine) AddRecord(ctx context.Context, dataSourceCode string, recordID string, recordDefinition string, flags int64) (string, error) {
	...
	return client.AddRecordResult, err
}
...
func (client *Szengine) DeleteRecord(ctx context.Context, dataSourceCode string, recordID string, flags int64) (string, error) {
	...
	if client.isTrace {
		...
		defer func() { client.traceExit(18, dataSourceCode, recordID, flags, err, time.Since(entryTime)) }()
	}
	if client.observers != nil {
		go func() {
			details := map[string]string{
	...
	return client.DeleteRecordResult, err
}
...
func (client *Szengine) FetchNext(ctx context.Context, exportHandle uintptr) (string, error) {
	...
	if client.isTrace {
		...
		defer func() { client.traceExit(32, exportHandle, client.FetchNextResult, err, time.Since(entryTime)) }()
	}
	if client.observers != nil {
		go func() {
			details := map[string]string{}
...
func (client *Szengine) FindNetworkByEntityID(ctx context.Context, entityList string, maxDegrees int64, buildOutDegree int64, maxEntities int64, flags int64) (string, error) {
	...
	if client.isTrace {
		entryTime := time.Now()
		client.traceEntry(37, entityList, maxDegrees, buildOutDegree)
		defer func() {
			client.traceExit(38, entityList, maxDegrees, buildOutDegree, client.FindNetworkByEntityIDResult, err, time.Since(entryTime))
		}()
	}
	if client.observers != nil {
		go func() {
...
func (client *Szengine) FindNetworkByRecordID(ctx context.Context, recordList string, maxDegrees int64, buildOutDegree int64, maxEntities int64, flags int64) (string, error) {
	...
	if client.isTrace {
		entryTime := time.Now()
		client.traceEntry(41, recordList, maxDegrees, buildOutDegree, maxEntities, flags)
		defer func() {
			client.traceExit(42, recordList, maxDegrees, buildOutDegree, maxEntities, flags, client.FindNetworkByRecordIDResult, err, time.Since(entryTime))
		}()
	}
	if client.observers != nil {
		go func() {
...
*/
func (client *Szengine) FindPahtByEntityID(ctx context.Context, startEntityID int64, endEntityID int64, maxDegrees int64, exclusions string, requiredDataSources string, flags int64) (string, error) {
	var err error = nil
	if client.isTrace {
		entryTime := time.Now()
		client.traceEntry(45, startEntityID, endEntityID, maxDegrees)
		defer func() {
			client.traceExit(46, startEntityID, endEntityID, maxDegrees, client.FindPathByEntityIDResult, err, time.Since(entryTime))
		}()
	}
	if client.observers != nil {
		go func() {
...
func (client *Szengine) FindPathByRecordID(ctx context.Context, startDataSourceCode string, startRecordID string, endDataSourceCode string, endRecordID string, maxDegrees int64, exclusions string, requiredDataSources string, flags int64) (string, error) {
	...
	if client.isTrace {
		entryTime := time.Now()
		client.traceEntry(49, startDataSourceCode, startRecordID, endDataSourceCode, endRecordID, maxDegrees)
		defer func() {
			client.traceExit(50, startDataSourceCode, startRecordID, endDataSourceCode, endRecordID, maxDegrees, client.FindPathByRecordIDResult, err, time.Since(entryTime))
		}()
	}
	if client.observers != nil {
		go func() {
...
func (client *Szengine) GetStats(ctx context.Context) (string, error) {
	...
	if client.isTrace {
		...
		defer func() { client.traceExit(140, client.GetStatsResult, err, time.Since(entryTime)) }()
	}
	if client.observers != nil {
		go func() {
			details := map[string]string{}
	...
	return client.GetStatsResult, err
}
...
func (client *Szengine) ReevaluateEntity(ctx context.Context, entityID int64, flags int64) (string, error) {
	...
	return client.ReevaluateEntityResult, err
}
...
func (client *Szengine) ReevaluateRecord(ctx context.Context, dataSourceCode string, recordID string, flags int64) (string, error) {
	...
	return client.ReevaluateRecordResult, err
}
...
func (client *Szengine) SearchByAttributes(ctx context.Context, attributes string, searchProfile string, flags int64) (string, error) {
	...
	if client.isTrace {
		...
		defer func() { client.traceExit(134, attributes, client.SearchByAttributesResult, err, time.Since(entryTime)) }()
	}
	if client.observers != nil {
		go func() {
			details := map[string]string{}
